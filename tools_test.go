...
var uploadTest = []struct {
	...
	{name: "allowed to rename", allowedTypes: []string{"image/jpeg", "image/png"}, renameFile: false, errorExpected: false},
...
var slugTests = []struct {
	...
	{name: "japanase string", s: "こんにちは世界", expected: "", errorExpected: true},
	{name: "japanase string and roman characters", s: "hello world こんにちは世界", expected: "hello-world", errorExpected: false},
}
...
func TestTools_Slugify(t *testing.T) {
	...
	for _, e := range slugTests {
		...
		if err != nil && !e.errorExpected {
			t.Errorf("%s: error received when none is expected: %s", e.name, err.Error())
		}
...
func TestTool_DownloadStaticFile(t *testing.T) {
	...
	fmt.Printf("res.Header[\"Content-Disposition\"]: %s\n", res.Header["Content-Disposition"])
...
var jsonTests = []struct {
	...
	{name: "good json", json: `{"foo": "bar"}`, errorExpected: false, maxSize: 1042, allowUnknown: false},
	{name: "badly formatted json", json: `{"foo": }`, errorExpected: true, maxSize: 1042, allowUnknown: false},
	{name: "incorrect type", json: `{"foo": 1}`, errorExpected: true, maxSize: 1042, allowUnknown: false},
	{name: "two json files", json: `{"foo": 1}{"alpha", "beta"}`, errorExpected: true, maxSize: 1042, allowUnknown: false},
	{name: "empty body", json: ``, errorExpected: true, maxSize: 1042, allowUnknown: false},
	{name: "syntax error in json", json: `{"foo": 1"`, errorExpected: true, maxSize: 1042, allowUnknown: false},
	{name: "unknown field", json: `{"foooo": "1"`, errorExpected: true, maxSize: 1042, allowUnknown: false},
	{name: "allow unknown fields in json", json: `{"foooo": "1"}`, errorExpected: false, maxSize: 1042, allowUnknown: true},
	{name: "missing field name", json: `{jack: "1"}`, errorExpected: true, maxSize: 1042, allowUnknown: true},
